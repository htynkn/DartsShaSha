...
public class DartsController extends IController {
    ...
    public DartsController(AssetManager assetManager, TextureAtlas.AtlasRegion region) {
        this.region = region;
        this.bing = assetManager.get("audio/bing.ogg", Sound.class);
    }
    ...
    public void AddDarts(Dart dart) {
        ...
        float r = (dart.getTarget().y - dart.getY()) / (dart.getTarget().x - dart.getX()); // 获取斜率
    ...
    public Boolean attackAlive(Actor target, Actor projectile) {
        Rectangle rectangle = new Rectangle(target.getX(), target.getY(), target.getWidth(),
                target.getHeight()); // 创建一个矩形
        return rectangle.contains(projectile.getX() + projectile.getWidth() / 2,
